<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Tabel 6.f.1 Waktu Tunggu Lulusan - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <a href="{{ route('waktu_tunggu_lulusan.index') }}" class="btn btn-outline-secondary mb-4 rounded-pill">
                <i class="bi bi-arrow-left me-2"></i>Kembali
            </a>

            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Update Waktu Tunggu Lulusan (Program Sarjana)</h5>
                
                <form action="{{ route('waktu_tunggu_lulusan.update', $data->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-3 bg-light rounded-3 mb-4 border">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" name="wt_kurang_3_bulan" class="form-control rounded-3" value="{{ $data->wt_kurang_3_bulan }}" min="0" required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label fw-semibold text-sm">3 ≤ WT ≤ 18 bulan</label>
                                <input type="number" name="wt_antara_3_18_bulan" class="form-control rounded-3" value="{{ $data->wt_antara_3_18_bulan }}" min="0" required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label fw-semibold text-sm">WT > 18 bulan</label>
                                <input type="number" name="wt_lebih_18_bulan" class="form-control rounded-3" value="{{ $data->wt_lebih_18_bulan }}" min="0" required>
                            </div>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-warning w-100 py-3 rounded-pill shadow-sm fw-bold">
                        <i class="bi bi-pencil-square me-2"></i>UPDATE DATA WAKTU TUNGGU
                    </button>
                </form>
            </div>
        </div>
