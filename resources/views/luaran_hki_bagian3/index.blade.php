<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tabel 6.e.3-3 Teknologi Tepat Guna, Produk - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Input Luaran Bagian-3 (Teknologi Tepat Guna, Produk)</h5>
                
                <form action="{{ route('luaran_hki_bagian3.store') }}" method="POST">
                    @csrf
                    
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-sm">Luaran Penelitian dan PkM</label>
                        <input type="text" name="luaran_penelitian" class="form-control rounded-3" placeholder="Contoh: Modul PLTS, Aplikasi Smart Village..." required>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="status" class="form-control rounded-3" placeholder="Contoh: TKT 3, TKT 5..." required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold text-sm">Nomor Sertifikat TKT</label>
                            <input type="text" name="nomor_sertifikat" class="form-control rounded-3" placeholder="Contoh: TKT-001/2023" required>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary w-100 py-3 rounded-pill shadow-sm fw-bold"
                        ...
                        <i class="bi bi-save me-2"></i>SIMPAN DATA TEKNOLOGI
                    </button>
                </form>
            </div>
            
            <div class="card shadow-sm border-0 rounded-4 p-4">
                <h6 class="fw-bold mb-3">Daftar Teknologi Tersimpan ({{ $data->count() }})</h6>
                <div class="table-responsive">
                    <table class="table table-sm table-hover align-middle text-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('luaran_hki_bagian3.edit', $item->id) }}" class="btn btn-sm text-warning p-0" title="Edit">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <form action="{{ route('luaran_hki_bagian3.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="6" class="text-muted py-3">Belum ada data Teknologi yang diinput.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
