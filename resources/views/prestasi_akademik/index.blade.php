<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tabel 6.c.1 Prestasi Akademik - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Input Prestasi Akademik Mahasiswa</h5>
                <form action="{{ route('prestasi_akademik.store') }}" method="POST">
                    @csrf
                    
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-sm">Nama Kegiatan</label>
                            <input type="text" name="nama_kegiatan" class="form-control rounded-3" placeholder="Contoh: Olimpiade Sains Nasional..." required>
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="prestasi_dicapai" class="form-control rounded-3" placeholder="Contoh: Juara 1, Medali Emas, Finalis..." required>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover align-middle text-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('prestasi_akademik.edit', $item->id) }}" class="btn btn-sm text-warning p-0" title="Edit">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <form action="{{ route('prestasi_akademik.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                            @csrf @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="text-muted py-3">Belum ada data prestasi akademik.</td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
