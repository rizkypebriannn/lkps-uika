<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tabel 4.f HKI (Hak Cipta dll) - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Input Luaran HKI (Hak Cipta, Desain Industri, dll.)</h5>
                <form action="{{ route('luaran_hki_hak_cipta.store') }}" method="POST">
                    @csrf
                    
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-sm">Judul Luaran Penelitian dan PkM</label>
                        <textarea name="judul_luaran" class="form-control rounded-3" rows="2" placeholder="Masukkan judul penelitian/hak cipta..." required></textarea>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="keterangan" class="form-control rounded-3" placeholder="Contoh: EC00202112345 (Kosongkan jika belum ada)">
                        </div>
                    </div>
    ...
    <i class="bi bi-save me-2"></i>SIMPAN DATA
            ...
            <div class="card shadow-sm border-0 rounded-4 p-4">
                <h6 class="fw-bold mb-3">Daftar Hak Cipta/Desain Tersimpan ({{ $hkis->count() }})</h6>
    ...
    <div class="d-flex justify-content-center gap-2">
        <a href="{{ route('luaran_hki_hak_cipta.edit', $item->id) }}" class="btn btn-sm text-warning p-0" title="Edit">
            <i class="bi bi-pencil-fill"></i>
        </a>
        <form action="{{ route('luaran_hki_hak_cipta.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
            @csrf @method('DELETE')
