<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tabel 3.c PkM DTPS - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Input Jumlah Judul PkM DTPS</h5>
                <form action="{{ route('pkm_dtps.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
    ...
    <i class="bi bi-save me-2"></i>SIMPAN DATA (Otomatis Kembali ke Dashboard)
            ...
            <div class="card shadow-sm border-0 rounded-4 p-4">
                <h6 class="fw-bold mb-3">Data Tersimpan ({{ $pkms->count() }})</h6>
    ...
    <div class="d-flex justify-content-center gap-2">
        <a href="{{ route('pkm_dtps.edit', $item->id) }}" class="btn btn-sm text-warning p-0" title="Edit">
            <i class="bi bi-pencil-fill"></i>
        </a>
        <form action="{{ route('pkm_dtps.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
            @csrf @method('DELETE')
