<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tabel 4.f HKI (Paten) - ') }} <span class="text-indigo-600">{{ auth()->user()->prodi->nama_prodi ?? '' }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-5 p-4">
                <h5 class="fw-bold mb-4 border-bottom pb-2">Input Luaran HKI (Paten / Paten Sederhana)</h5>
                <form action="{{ route('luaran_hki_paten.store') }}" method="POST">
                    @csrf
                    
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-sm">Judul Luaran Penelitian dan PkM</label>
                        <textarea name="judul_luaran" class="form-control rounded-3" rows="2" placeholder="Masukkan judul penelitian/paten..." required></textarea>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-sm">Nomor Paten (Granted)</label>
                            <input type="text" name="nomor_paten" class="form-control rounded-3" placeholder="Contoh: IDS000001234 (Kosongkan jika belum ada)">
                        </div>
                    </div>
            ...
            <div class="card shadow-sm border-0 rounded-4 p-4">
                <h6 class="fw-bold mb-3">Daftar Paten Tersimpan ({{ $hkis->count() }})</h6>
